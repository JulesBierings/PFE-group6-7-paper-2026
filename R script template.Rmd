...
```{r Read the raw dataset}
raw_dataset <- read.csv("data/raw_dataset.csv")
```

```{r Huizenprijzen Dataset}
...
landelijk_index <- landelijk %>%
  arrange(Year) %>%
  mutate(
    WOZ_index = GemiddeldeWOZ / GemiddeldeWOZ[Year == 2011] * 100,
    Inkomen_index = InkomenWerknemer / InkomenWerknemer[Year == 2011] * 100
  ) %>%
...
ggplot(landelijk_index, aes(x = Year, y = Index, color = Variable)) +
  ...
  labs(
    ...
    subtitle = "Indexed values, 2011 = 100",
    x = "Year",
    y = "Index value",
    color = "Variable"
  ) +
